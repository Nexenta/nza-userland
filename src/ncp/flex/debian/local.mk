...
debian/stamp/install/flex:
	...
	$(MAKE)              $(INT_INSTALL_TARGET)  prefix=$(TMPTOP)/usr/gnu \
	                     infodir=$(INFODIR)     mandir=$(MANDIR)     \
                              INSTALL_PROGRAM="$(install_program)"
	$(install_file)      debian/libfl.shared_object $(CURDIR)/debian/flex/usr/gnu/lib/libfl.so
	$(MAKE) install      -C po                  infodir=$(INFODIR) prefix=$(TMPTOP)/usr/gnu
	$(MAKE) install-info -C doc                 infodir=$(INFODIR) prefix=$(TMPTOP)/usr/gnu
	#ln -s                flex                   $(BINDIR)/flex++
	ln -s                flex                   $(CURDIR)/debian/flex/usr/gnu/bin/flex++
...
# Make sure the copyright file is not compressed
	...
	ln -s                flex                   $(CURDIR)/debian/flex/usr/gnu/bin/lex
	ln -s                libfl.a                $(CURDIR)/debian/flex/usr/gnu/lib/libl.a
	@test -d debian/stamp/install || mkdir -p debian/stamp/install
	@echo done > $@
